...
{
    public class ObjectPoolManager :MonoSingleton<ObjectPoolManager>, IObjectPoolManager
    {
        ...
        public bool HasObjectPool(Type objectType)
        {
            if (objectType == null)
            {
                throw new LException("Object type is invalid.");
            }

            if (!typeof(ObjectBase).IsAssignableFrom(objectType))
            {
                throw new LException(string.Format("Object type '{0}' is invalid.", objectType.FullName));
            }
        ...
        public bool DestroyObjectPool<T>(IObjectPool<T> objectPool) where T : ObjectBase
        {
            if (objectPool == null)
            {
                throw new LException("Object pool is invalid.");
            }
        ...
        private IObjectPool<T> InternalCreateObjectPool<T>(string name, bool allowMultiSpawn, int capacity, float expireTime, int priority) where T : ObjectBase
        {
            ...
            {
                throw new LException(string.Format("Already exist object pool '{0}'.", Utility.Text.GetFullName<T>(name)));
            }
        ...
        private ObjectPoolBase InternalCreateObjectPool(Type objectType, string name, bool allowMultiSpawn, int capacity, float expireTime, int priority)
        {
            ...
            {
                throw new LException(string.Format("Already exist object pool '{0}'.", Utility.Text.GetFullName(objectType, name)));
            }
